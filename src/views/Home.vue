<template>
  <div class="home">
    ...
    <div v-else class="dashboard">
      ...
      <div class="section">
        ...
        <div class="actions-grid">
          ...
          <router-link to="/admin/social-feed" class="action-card">
    ...
    <div v-if="isAuthenticated === undefined || isAuthenticated === null" class="loading">
      <div class="loading-spinner"></div>
      <p>Loading TimeGrid...</p>
    </div>
  </div>
</template>
...
const isAuthenticated = computed(() => {
  // Safety check - ensure we have a boolean value
  const val = authStore.isAuthenticated
  return typeof val === 'boolean' ? val : false
})

const isHR = computed(() => {
  const admin = authStore.isAdmin
  const hr = authStore.isHR
  return !!(admin || hr)
})

const isEmployee = computed(() => {
  const emp = authStore.isEmployee
  return !!(emp)
})
...
.hero-bg {
  ...
  inset: 0;
  background: 
    radial-gradient(ellipse at top, rgba(0, 110, 91, 0.15) 0%, transparent 50%),
...
.gradient-text {
  background: linear-gradient(135deg, #006e5b, #3b82f6);
...
.action-icon.chat { background: rgba(59, 130, 246, 0.1); color: #3b82f6; }
.action-icon.feed { background: rgba(168, 85, 247, 0.1); color: #a78bfa; }
